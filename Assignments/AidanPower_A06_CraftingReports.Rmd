...
```{r Ozone_2018,echo=FALSE,fig.height=3,fig.cap="2018 monthly Ozone levels"}

Ozone_2018 <- filter(epa_data, Year==2018) %>%
  drop_na(Ozone)
#Isolated 2018 data from rest of data frame

Ozone_2018$Month <-  factor(Ozone_2018$Month,levels = 1:12,labels=month.abb)
#Changed the months from numbers to abrevs (in correct order)

ggplot(Ozone_2018,aes(x=Month, y=Ozone,align=c("l"))) +
  geom_boxplot() +
  labs(title="Monthly Ozone Concentrations (2018)",x="Month",y=expression(Concentration ~ (mu * g / m^3))) 
#Plotted Ozone Conc vs. Month as box plot

```

```{r Ozone_2019,echo=FALSE,fig.height=3,fig.cap="2019 monthly Ozone levels"}

Ozone_2019 <- filter(epa_data, Year==2019) %>%
  drop_na(Ozone)
#Isolated 2019 data from rest of data frame

Ozone_2019$Month <-  factor(Ozone_2019$Month,levels = 1:12,labels=month.abb)
#Changed the months from numbers to abrevs (in correct order)

ggplot(Ozone_2019,aes(x=Month, y=Ozone,align=c("l"))) +
  geom_boxplot() +
