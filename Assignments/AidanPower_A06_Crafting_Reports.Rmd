...
#table <- data.frame(County=summary_PM25$COUNTY, Concentration = signif(summary_PM25$mean_PM2.5, digits = 4))
...
```{r,echo=FALSE,fig.height=3,fig.cap="2018 monthly Ozone levels"}
...
ggplot(Ozone_2018,aes(x=as.factor(Month), y=Ozone)) +
  geom_boxplot() +
  labs(title="Monthly Ozone Concentrations (2018)",x="Month",y="Ozone($\\mu g/m^3$)") 
  

```

```{r,echo=FALSE,fig.height=3,fig.cap="2019 monthly Ozone levels"}
Ozone_2019 <- filter(epa_data, Year==2019) %>%
  drop_na(Ozone)

ggplot(Ozone_2019,aes(x=as.factor(Month), y=Ozone),align=c("l")) +
  geom_boxplot() +
  labs(title="Monthly Ozone Concentrations (2019)",x="Month",y="Ozone(ug/m^3)")
