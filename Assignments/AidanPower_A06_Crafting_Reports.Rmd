...
getwd()
library(knitr)
table <- data.frame(Item=c('File Name','Date','Source'),Value=c('EPAair_O3_PM25_NC1819_Processed.csv','2018-2019','EPA Air Quality System (AQS)'))
kable(table,align = c("r","l"), caption = "EPA Air Quality")
...
table <- data.frame(County=summary_PM25$COUNTY, "$\\mu g/m^3$" = signif(summary_PM25$mean_PM2.5, digits = 4))
kable(table,align = c("r","l"), caption = "Mean Particulates (2.5mm)")

table <- data.frame(County=summary_Ozone$COUNTY, "$\\mu g/m^3$" = signif(summary_Ozone$mean_Ozone, digits = 4))
kable(table,align = c("r","l"), caption = "Mean Ozone")
...
```{r}
Ozone_2018 <- read.csv(here('Data','Processed_KEY','EPAair_O3_NC2018_processed.csv'))

Month=month(Ozone_2018$Date)
#Where to find how to add month column 

ggplot(Ozone_2018,aes(x=))
  geom_boxplot()

```

```{r}
Ozone_2019 <- read.csv(here('Data','Processed_KEY','EPAair_O3_NC2019_processed.csv'))
